...
#[post("/delete_column")]
async fn delete_column(req: web::Json<DeleteColumnRequest>) -> impl Responder {
    // Retrieve the table
    let table_result: Result<Vec<Table>, Error> = DB.select((&req.table_name).clone()).await;
        ...
        Some(mut table) => {
            ...
            if req.col < table.data[0].len() {
                ...
                match update_result {
                    ...
                    Err(e) =>
                        HttpResponse::InternalServerError().body(
                            format!("Error deleting column: {}", e)
                        ),
                }
            } else {
                HttpResponse::BadRequest().body("Invalid column index")
